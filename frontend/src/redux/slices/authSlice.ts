...
export const verifyOTP = createAsyncThunk(
    ...
    async (otp: string, thunkAPI) => {
        try {
            ...
            const data = getAxiosErrorData(error);
            return thunkAPI.rejectWithValue(data?.error?.message || 'OTP verification failed');
        }
    }
);
...
export const registerUserRole = createAsyncThunk(
    ...
    async ({ role, roleData }: { role: string; roleData: any }, thunkAPI) => {
        try {
            ...
            const data = getAxiosErrorData(error);
            return thunkAPI.rejectWithValue(data?.error?.message || 'Role registration failed');
        }
    }
);
...
const authSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(verifyOTP.fulfilled, (state, action: PayloadAction<UserData>) => {
                Object.assign(state, action.payload, {
                    isAuthenticated: true,
                    loading: false,
                    error: '',
            ...
            .addCase(registerUserRole.fulfilled, (state, action: PayloadAction<UserData>) => {
                Object.assign(state, action.payload, {
                    isAuthenticated: true,
                    loading: false,
                    error: '',
